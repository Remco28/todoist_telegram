...
- Telegram task-list presentation is hierarchy-aware now too: open-task views group `Projects`, `Tasks`, and root `Subtasks`, nested children render under their parent with indentation, and flat list views use a light `Project:` cue so projects are visually distinct without adding emoji noise.
